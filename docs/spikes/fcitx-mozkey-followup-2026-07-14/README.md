...
- fixed Mozc sidecarを同じSwift executable / `candidates()`境界で測ったwarm latencyでも
  ...
  overlay、learning parity、実serverへの外部SIGKILL/partial-frame EOF回復が未完のため、
  default切替の根拠にはしない。
...
Mozc-only coreの値ではなく現在のproduct実装全体の値である。外部SIGKILL/partial-frame EOFの
実server故障注入とFcitxを含むsoakも別gateとして残る。
...
2つ目のhelperで成功した。これによりpost-handshake timeout回復は完了とし、外部`SIGKILL`と
partial-frame EOFは引き続き別の故障注入gateとする。
...
focused `GrimodexMozcSidecarTests`は24 testsで、通常実行は23 pass + actual bundle smoke
1 optional skip、bundle指定実行では24 passした。coverageはexact opt-in、Unicode boundary、
...
no-replay、real server / Protocol v2でのEOF/post-handshake timeout cached failureとfresh
respawn、response cross-field mismatch、zero target、timeout/oversize/request mismatchを含む。
...
4. **完了:** helper EOF/timeout時にrequestを再送せずcompositionを保持し、同じrequest IDを
   cached failureとして返し、fresh requestだけでrespawnする経路を実server / Protocol v2で通した
...
8. **部分完了:** 同じadapter境界のwarm latency、PSS、150,000変換soakに加え、
   長寿命server / Protocol v2境界のsteady-state 1,500変換とEOF/timeout回復は完了。
   外部SIGKILL/partial-frame EOF、Fcitx込みsoak、交互cycleは未完
