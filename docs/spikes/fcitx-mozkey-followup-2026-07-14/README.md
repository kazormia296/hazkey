...
- fixed Mozc sidecarを同じSwift executable / `candidates()`境界で測ったwarm latencyでも
  ...
  overlay、learning parity、実serverへの外部SIGKILL回復が未完のため、default切替の
  根拠にはしない。
...
Mozc-only coreの値ではなく現在のproduct実装全体の値である。外部SIGKILLの実server故障注入と
Fcitxを含むsoakも別gateとして残る。
...
2つ目のhelperで成功した。これによりpost-handshake timeout回復は完了とし、外部`SIGKILL`は
引き続き別の故障注入gateとする。
...
focused `GrimodexMozcSidecarTests`は25 testsで、通常実行は24 pass + actual bundle smoke
1 optional skip、bundle指定実行では25 passした。coverageはexact opt-in、Unicode boundary、
...
cached failure/fresh respawn、response cross-field mismatch、zero target、timeout/oversize/
request mismatchを含む。
...
8. **部分完了:** 同じadapter境界のwarm latency、PSS、150,000変換soakに加え、
   長寿命server / Protocol v2境界のsteady-state 1,500変換と通常/partial-frame EOF、timeout回復は
   完了。外部SIGKILL、Fcitx込みsoak、交互cycleは未完
