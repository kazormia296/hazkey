...
- fixed Mozc sidecarを同じSwift executable / `candidates()`境界で測ったwarm latencyでも
  ...
  overlay、learning parity、実serverのSIGKILL/timeout回復が未完のため、default切替の根拠にはしない。
...
`exit(0)`由来のEOFを対象にした実server testであり、外部`SIGKILL`、post-handshake timeout、
...
focused `GrimodexMozcSidecarTests`は23 testsで、通常実行は22 pass + actual bundle smoke
1 optional skip、bundle指定実行では23 passした。coverageはexact opt-in、Unicode boundary、
...
no-replay、real server / Protocol v2でのcached failureとfresh respawn、response cross-field
mismatch、zero target、timeout/oversize/request mismatchを含む。
...
4. **完了:** helper EOF時にrequestを再送せずcompositionを保持し、同じrequest IDを
...
8. **部分完了:** 同じadapter境界のwarm latency、PSS、150,000変換soakに加え、
   長寿命server / Protocol v2境界のsteady-state 1,500変換とEOF回復は完了。SIGKILL/timeout、
