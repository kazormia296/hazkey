...
- fixed Mozc sidecarを同じSwift executable / `candidates()`境界で測ったwarm latencyでも
  ...
  中央v2 conversion-only contract、Fcitx込みの正式15万回soakとpaired交互A/Bが未完のため、
  default切替の根拠にはしない。bounded Fcitx soakは3 fresh cycle×100変換で完了した。
...
### Fcitx full-stack soak / lifecycle cycle

display-freeな実Fcitx `testfrontend`からaddon→Protocol v2→server→actual B0 helperまでを通す
Mozc専用CTestを、通常Hazkey testとは別のdefault-off optionとして追加した。configure時に
検証済みと確定したgeneration pathだけを受け取り、test開始時にhost PINGで再検証してから、
test固有の0700 root内へ0555/0444のread-only runtime generationを作る。ambientの
`FCITX*`、`GRIMODEX_IME*`、`GGML*`、`LD_LIBRARY_PATH`は継承せず、build-treeのbackendと
private helper/dataだけを明示する。

同一Fcitx/server/InputContextで`きょう`と`あした`の変換・候補表示・commit・UI消去を
100回反復し、これをfresh server/helperで3 cycle実行した。private wrapperがexec直前に
PIDと`/proc` start-timeを0600のappend-only auditへ記録し、各cycleのserver/helper launchが
正確に1回、観測identityとhelperの観測最大同時数も1であることを確認した。全300変換後に
server/helper残留とprivate test root残留がないことを確認した。通常Hazkey full-stack、Mozc full-stack、Mozc soakは
それぞれ2.73秒、3.41秒、9.48秒で通過した。auditのPID/start-time一致とpidfdを使うため、
PID再利用時に別processを停止しない。これはboundedな実UI経路のsoakとclean
lifecycle再生成の証拠であり、Fcitx経路での150,000連続変換、server SIGKILL注入、
paired交互A/B latency測定を置き換えない。
...
strict import、通常release build、全Swift回帰278件（12 optional skip、失敗0）、
actual bundle指定のfocused 30件、通常CTest 17件、Mozc opt-in時CTest 19件、
実stage検証を含む71件のpackaging contractは
成功した。一方、このactual
...
cmake -S . -B build-grimodex \
  ...
  -DGRIMODEX_ENABLE_MOZC_INTEGRATION_TESTS=ON \
  -DGRIMODEX_MOZC_SOAK_ITERATIONS=100 \
  -DGRIMODEX_MOZC_RESTART_CYCLES=3
cmake --build build-grimodex --parallel 4

ctest --test-dir build-grimodex --output-on-failure \
  -R '^hazkey-fcitx-(full-stack|mozc-(full-stack|soak))-test$'
...
8. **部分完了:** 同じadapter境界のwarm latency、PSS、150,000変換soakに加え、
   ...
   外部SIGKILL回復、実Fcitx同一session 100変換×fresh 3 cycle、cycleごとの監査済み
   server/helper単一起動・単一identity・終了確認は完了。Fcitx経路の正式150,000回とpaired交互A/Bは未完
