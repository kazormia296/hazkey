...
3 run間で同一のproducer、GGUF、辞書、空文脈sidecar、`top_k=5`を使い、既知の許容経路1,360件を
再取得した。
これはNative境界仮説の探索用であり、規則探索に既に使った`diagnostic_only`集合なので正式採否には使わない。

| 方式 | Boundary@1 | Boundary@5 | 条件付きSurface@1 | End-to-End@1 |
|---|---:|---:|---:|---:|
| 分離境界 | 545/1,360 (40.07%) | 545/1,360 (40.07%) | 147/158 (93.04%) | 147/533 (27.58%) |
| Native Zenzai | 559/1,360 (41.10%) | 593/1,360 (43.60%) | 152/162 (93.83%) | 152/533 (28.52%) |
| Mozc固定 | 715/1,360 (52.57%) | 715/1,360 (52.57%) | 198/208 (95.19%) | 198/533 (37.15%) |

Nativeは分離境界に対しBoundary@1で28件を救済し14件を悪化させ、純増14件だった。End-to-End@1は
14件救済、9件悪化の純増5件である。したがって、境界をZenzai OFFの別コンバーターへ分離した設計が
Nativeの間接効果を一部捨てていたことは確認できた。ただし改善後もMozc固定境界との差は大きく、Nativeへ
無条件切替する根拠にはならない。

この集合には通常のIME入力経路を表さないdirect-ASCII混在が565件ある。そこでは分離境界とNativeがともに
Boundary@1 14/565で、Nativeの救済・悪化は各1件だった。一方、ASCIIを含まない795件では分離境界
531/795 (66.79%)、Native 545/795 (68.55%)で、救済27件、悪化13件となった。全体の純増14件はこの
非ASCII群から生じている。今後はruntimeで観測可能な入力shapeとして両群を別集計し、direct-ASCII群を
通常かな入力の品質分母へ混ぜない。

Zenzai terminal `no_candidate`は分離境界365/2,720 request、Native 362/1,360 request、Mozc固定
8/1,360 requestだった。特にASCII混在で集中しており、formal blockerとして残す。別processを逐次実行した
診断値ではレイテンシ中央値が分離境界77.27ms、Native 57.75ms、Mozc固定23.05msだったが、ランダム化した
製品UI測定ではないため方式間の速度差を採否値にはしない。入力shape別集計を含む完全な評価artifactは
`build-grimodex/zenzai-boundary-policy-quality-1360-20260717-v1/evaluation-v2.json`に保存した。

#### AJIMEE 100件での左文脈あり全文変換診断

境界とは別に、Zenzaiの本来の強みである左文脈付き全文変換を測った。AJIMEE-Benchの固定revision
`401666cd56d1a570c2021798b64b6da4396bfd45`、`JWTD_v2/v1/evaluation_items.json`から、上流に左文脈が
存在する100件をそのまま抽出した。raw snapshotのSHA-256は
`e9eb668fd6aa14b1e26436f429b5550108af0a1dfd443b8cea0bcb3ab3028fca`、ライセンスはCC BY-SA 3.0である。
Mozc/Hazkey候補やscoreを見ずに入力、複数参照表層、自然左文脈、全件空文字の対照sidecarを生成した。

ABProbe v7へprobe専用`full_composition` modeを追加し、`adapter.candidates()`から入力全体と同じ
`consuming_count`の候補だけを保持した。`conversion_path=full_composition_candidates`、Zenzai requestは
1件、境界指標は非集計である。空文脈と自然左文脈は、同じrelease producer、GGUF、辞書、入力、
`top_k=5`で逐次取得した。

| 指標 | 空文脈 | 自然左文脈 | 対比較 |
|---|---:|---:|---:|
| Accuracy@1 | 59/100 (59%) | 67/100 (67%) | 救済10、悪化2、純増8 |
| Accuracy@5 | 66/100 (66%) | 74/100 (74%) | 救済10、悪化2、純増8 |
| MRR@5 | 0.6200 | 0.7017 | +0.0817 |

左文脈でTop-1が8ポイント改善しており、空文脈の先頭文節評価だけではZenzaiの価値を判断できないことが
確認できた。一方、Zenzai terminal `no_candidate`は空文脈2件から自然左文脈7件へ増えた。失敗caseでも
辞書fallback候補は品質分母に残しているため、この件数を黙って除外せずformal blockerとして扱う。
...
- ABProbe v7の左文脈経路と空文脈対照は実装し、AJIMEE Silver 100件の全文対取得も完了したが、
  左文脈条件付きの未知Gold holdoutはない。自然左文脈でterminal `no_candidate`が7/100件あり、
  request単位の実行outcomeは公開したが、学習tokenがTop-1へ寄与した候補単位の証拠はscoreと別フィールドで
  公開していない。
- runtimeで使用可能な入力カテゴリ定義と分類器が固定されていない。
- 全文変換用ABProbeは追加したが境界列を公開しないため、全文Acceptable Path Accuracyや全境界F1には
  引き続きrunner拡張が要る。
