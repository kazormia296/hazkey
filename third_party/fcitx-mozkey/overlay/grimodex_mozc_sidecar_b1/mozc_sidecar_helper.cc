...
#include "grimodex_mozc_sidecar_b1/mozc_sidecar.pb.h"
...
using ::hazkey::mozc_sidecar_b1::BuildB1Candidates;
using ::hazkey::mozc_sidecar_b1::SegmentCandidate;
...
SidecarResponse Convert(const SidecarRequest& sidecar_request,
    ...
    const bool is_full_reading_target =
        sidecar_request.target_key_size() != 0 &&
        sidecar_request.target_key_size() == reading_size;
    std::vector<std::vector<SegmentCandidate>> natural_candidates;
    if (is_full_reading_target) {
        // ResizeSegment mutates the natural boundaries and discards the
        // sentence-level alternatives B1 needs. Copy only candidate values and
        // ranks before resize; copying Segments can share Mozc's mutable
        // internals and change the forced B0 result.
        natural_candidates.reserve(segments.conversion_segments_size());
        for (size_t segment_index = 0;
             segment_index < segments.conversion_segments_size();
             ++segment_index) {
            const mozc::Segment& natural_segment =
                segments.conversion_segment(segment_index);
            const size_t natural_candidate_count =
                std::min(natural_segment.candidates_size(),
                         static_cast<size_t>(max_candidates));
            std::vector<SegmentCandidate>& destination =
                natural_candidates.emplace_back();
            destination.reserve(natural_candidate_count);
            for (size_t candidate_index = 0;
                 candidate_index < natural_candidate_count;
                 ++candidate_index) {
                const mozc::Segment::Candidate& source =
                    natural_segment.candidate(
                        static_cast<int>(candidate_index));
                destination.push_back({source.value, candidate_index});
            }
        }
    }
    ...
    if (is_full_reading_target && candidate_count > 0) {
        // Candidate zero is deliberately byte-for-byte identical to B0's
        // forced first candidate. Natural full-sentence alternatives only
        // start at response position two.
        const mozc::Segment::Candidate& forced = segment.candidate(0);
        ::hazkey::mozc_sidecar::Candidate* candidate =
            response.add_candidates();
        candidate->set_value(forced.value);
        candidate->set_description(forced.description);
        candidate->set_consumed_key_size(segment_key_size);

        std::vector<SegmentCandidate> forced_candidates;
        forced_candidates.reserve(candidate_count);
        for (size_t index = 0; index < candidate_count; ++index) {
            forced_candidates.push_back(
                {segment.candidate(static_cast<int>(index)).value, index});
        }

        std::vector<std::string> values = BuildB1Candidates(
            forced_candidates, natural_candidates,
            static_cast<size_t>(max_candidates));
        for (size_t index = 1; index < values.size(); ++index) {
            ::hazkey::mozc_sidecar::Candidate* candidate =
                response.add_candidates();
            candidate->set_value(values[index]);
            for (size_t forced_index = 1;
                 forced_index < candidate_count; ++forced_index) {
                const mozc::Segment::Candidate& forced_alternative =
                    segment.candidate(static_cast<int>(forced_index));
                if (forced_alternative.value == values[index]) {
                    candidate->set_description(
                        forced_alternative.description);
                    break;
                }
            }
            candidate->set_consumed_key_size(segment_key_size);
